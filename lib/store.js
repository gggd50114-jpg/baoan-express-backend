...
function buildSeedData() {
    ...
    return {
        ...
        settings: { showTableToViewers: true },
        siteAssets: { bannerUrl: null, logoUrl: null }, // ảnh Tết tùy chỉnh (tải lên qua imgbb) - null = dùng ảnh mặc định trong /public/assets
        media: { type: null, tiktokUrl: null, videoUrl: null }, // video Tết: dán link TikTok hoặc tải video từ máy tính lên
...
function readDb() {
    ...
    // File db.json cũ (tạo trước khi có tính năng đổi ảnh Tết qua imgbb) sẽ không có
    // trường "siteAssets" - mặc định null (dùng ảnh gốc trong /public/assets).
    if (!data.siteAssets || typeof data.siteAssets !== "object") {
        data.siteAssets = { bannerUrl: null, logoUrl: null };
    }
    // File db.json cũ (tạo trước khi có tính năng video/TikTok Tết) sẽ không có trường "media"
    if (!data.media || typeof data.media !== "object") {
        data.media = { type: null, tiktokUrl: null, videoUrl: null };
    }
    return data;
}
