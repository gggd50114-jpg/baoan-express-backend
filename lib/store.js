...
const SEED_ROUTES_PATH = path.join(__dirname, "..", "seed-data", "seed_routes.json");
const SEED_WEIGHT_PATH = path.join(__dirname, "..", "seed-data", "seed_weightBrackets.json");
...
function buildSeedData() {
    ...
    return {
        ...
        settings: { showTableToViewers: true, bannerYoutubeUrl: null, bannerVideoZoom: 2.2 },
        bannerImageUrl: null,
        bannerVideoUrl: null,
...
function readDb() {
    ...
    if (!data.settings || typeof data.settings.showTableToViewers !== "boolean") {
        data.settings = { showTableToViewers: true, bannerYoutubeUrl: null, bannerVideoZoom: 2.2 };
    }
    // File db.json cũ chưa có tính năng "video YouTube cho khung banner"
    if (typeof data.settings.bannerYoutubeUrl !== "string" && data.settings.bannerYoutubeUrl !== null) {
        data.settings.bannerYoutubeUrl = null;
    }
    // File db.json cũ chưa có tính năng "chỉnh mức zoom video banner" - mặc định 2.2 (phù hợp video dọc TikTok/Shorts)
    if (typeof data.settings.bannerVideoZoom !== "number" || data.settings.bannerVideoZoom < 1 || data.settings.bannerVideoZoom > 4) {
        data.settings.bannerVideoZoom = 2.2;
    }
    // File db.json cũ (tạo trước khi có tính năng đổi ảnh banner qua ImgBB)
    // sẽ không có trường "bannerImageUrl" - mặc định null để dùng ảnh banner
    // tĩnh có sẵn trong public/assets.
    if (typeof data.bannerImageUrl !== "string") {
        data.bannerImageUrl = null;
    }
    // File db.json cũ chưa có tính năng "tải video từ thiết bị lên cho khung banner"
    // (lưu ngoài trên Cloudinary, giống cách ảnh banner lưu trên ImgBB).
    if (typeof data.bannerVideoUrl !== "string") {
