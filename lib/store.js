...
function buildSeedData() {
    ...
    return {
        ...
        settings: { showTableToViewers: true, bannerImageUrl: "", logoImageUrl: "" },
...
function readDb() {
    ...
    // Đảm bảo luôn có 2 trường ảnh (rỗng = dùng ảnh mặc định trong /public/assets),
    // để db.json cũ (tạo trước khi có tính năng đổi ảnh qua ImgBB) không bị lỗi thiếu trường.
    if (typeof data.settings.bannerImageUrl !== "string") data.settings.bannerImageUrl = "";
    if (typeof data.settings.logoImageUrl !== "string") data.settings.logoImageUrl = "";
    return data;
}
