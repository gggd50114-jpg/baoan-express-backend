...
function buildSeedData() {
    ...
    return {
        ...
        settings: { showTableToViewers: true, tetGallery: [] },
...
function readDb() {
    ...
    // File db.json cũ (tạo trước khi có tính năng thư viện ảnh mừng Xuân) sẽ
    // không có trường "tetGallery" - mặc định mảng rỗng để không lỗi ở client.
    if (!Array.isArray(data.settings.tetGallery)) {
        data.settings.tetGallery = [];
    }
    return data;
}
