...
// URL ảnh banner/logo (do ImgBB trả về sau khi upload) - chỉ chấp nhận http/https,
// KHÔNG cho phép "javascript:", "data:" hay các scheme khác (chặn XSS qua thuộc tính src).
...
// settings tuỳ chọn khi lưu - object với các cờ/đường dẫn hiển thị.
// Whitelist nghiêm ngặt: chỉ nhận đúng các trường đã biết, mọi trường lạ bị bỏ qua.
function validateSettings(settings, current) {
    const base = current && typeof current === "object" ? current : { showTableToViewers: true };
    if (settings === undefined || settings === null || typeof settings !== "object" || Array.isArray(settings)) {
        return { ok: true, value: base };
    }
    const next = { ...base };
    if (typeof settings.showTableToViewers === "boolean") {
        next.showTableToViewers = settings.showTableToViewers;
    }
    if (settings.bannerImageUrl !== undefined) {
        if (settings.bannerImageUrl === "") {
            next.bannerImageUrl = ""; // rỗng = khôi phục ảnh mặc định
        } else if (isValidImageUrl(settings.bannerImageUrl)) {
            next.bannerImageUrl = settings.bannerImageUrl;
        } else {
            return { ok: false, error: `"settings.bannerImageUrl" phải là URL http/https hợp lệ.` };
        }
    }
    if (settings.logoImageUrl !== undefined) {
        if (settings.logoImageUrl === "") {
            next.logoImageUrl = "";
        } else if (isValidImageUrl(settings.logoImageUrl)) {
            next.logoImageUrl = settings.logoImageUrl;
        } else {
            return { ok: false, error: `"settings.logoImageUrl" phải là URL http/https hợp lệ.` };
        }
    }
